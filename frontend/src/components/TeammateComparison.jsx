...
function H2HBar({ leftVal, rightVal, leftCode, rightCode, label }) {
  ...
  return (
    <div className="mb-2">
      <div className="text-[10px] text-label-tertiary uppercase tracking-wider mb-0.5 text-center">{label}</div>
      <div className="flex items-center gap-1.5 text-xs font-mono">
        <span className={`w-8 text-right ${leftVal > rightVal ? 'text-green-400 font-bold' : 'text-label-tertiary'}`}>
          {leftVal}
        </span>
        <div className="flex-1 h-4 bg-glass-border/50 rounded-sm overflow-hidden flex">
          <div
        ...
        <span className={`w-8 ${rightVal > leftVal ? 'text-green-400 font-bold' : 'text-label-tertiary'}`}>
          {rightVal}
        </span>
      </div>
    </div>
...
function StatRow({ leftVal, rightVal, label, format = v => v, lowerBetter = false }) {
  ...
  return (
    <div className="flex items-center text-xs py-0.5">
      <span className={`w-16 text-right font-mono ${leftWins ? 'text-green-400 font-bold' : 'text-label-tertiary'}`}>
        {leftVal != null ? format(leftVal) : '-'}
      </span>
      <span className="flex-1 text-center text-[10px] text-label-tertiary uppercase">{label}</span>
      <span className={`w-16 font-mono ${rightWins ? 'text-green-400 font-bold' : 'text-label-tertiary'}`}>
    ...
    <div className="glass rounded-xl overflow-hidden">
      ...
      <div className="p-3">
        ...
        <div className="flex items-center text-sm font-bold mb-2">
          ...
          <span className="flex-1 text-center text-[10px] text-label-tertiary">vs</span>
        ...
        <div className="border-t border-glass-border mt-2 pt-2">
          ...
          {pointsPct && (
            <div className="text-[10px] text-label-tertiary text-center">Gap: {pointsDiff} pts ({pointsPct}%)</div>
          )}
...
export default function TeammateComparison({ year }) {
  ...
  if (!data?.comparisons?.length) {
    return <p className="text-sm text-label-tertiary text-center py-4">No teammate data available</p>
  }
