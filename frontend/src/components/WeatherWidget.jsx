...
function getRainColor(rainfall) {
  if (rainfall == null) return '#8888AA'
...
export default function WeatherWidget({ weather }) {
  ...
  const items = [
    ...
    { icon: Droplets, label: 'Humidity', value: weather.humidity != null ? `${weather.humidity}%` : '-' },
    {
      ...
      extra: windDir != null ? (
        <Navigation className="w-3 h-3 inline-block ml-1" style={{ transform: `rotate(${windDir}deg)`, color: '#8888AA' }} title={`${windDir}°`} />
      ) : null,
    },
    ...
    <div className="grid grid-cols-5 gap-2">
      {items.map(({ icon: Icon, label, value, color, extra }) => (
        <div key={label} className="text-center">
          <Icon className="w-4 h-4 mx-auto mb-1" style={{ color: color || '#8888AA' }} />
