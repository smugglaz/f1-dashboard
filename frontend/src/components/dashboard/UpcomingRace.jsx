  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {/* Race name + countdown */}
        <div>
          <div className="text-title-2">{race.name}</div>
          <div className="text-title-1 font-mono text-f1-red mt-1">{countdown}</div>
        </div>
        ...
        {schedule?.length > 0 && (
          <div className="space-y-1">
            <p className="text-caption-2 uppercase tracking-wider">Schedule</p>
            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs">
              {schedule.map((s, i) => (
                <div key={i} className="flex justify-between">
                  <span className="text-label-tertiary">{s.session}</span>
